class Solution {
    public boolean canFinish(int numCourses, int[][] prerequisites) {
        ...
        private boolean dfs(int course, Set<Integer> visiting, Map<Integer, List<Integer>> map) {
            if(visiting.contains(course)) return false; // find a loop
            if(map.get(course).isEmpty()) return true;  // no prereqs anymore
            
            visiting.add(course);
            // dfs until we find a loop - F / find no prereqs - T
